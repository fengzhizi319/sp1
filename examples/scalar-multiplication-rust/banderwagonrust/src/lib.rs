...
pub fn try_reduce_to_element(bytes: &[u8]) -> Option<Element> {
    ...
    // TODO So we reverse it here, to be interopable with the banderwagonrust specs which needs big endian bytes

    bytes.reverse();

    // Deserialize the x-coordinate to get a valid banderwagonrust element
